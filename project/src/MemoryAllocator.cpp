...
void MemoryAllocator::ispisiMiPraznu()
{
    for(FreeMem *pom = MemoryAllocator::freeMemHead; pom != nullptr; pom = pom->next)
    {
        //printInteger(pom->size);
    }
}

void MemoryAllocator::ispisiMiPunu()
{
    for(FreeMem *pom = MemoryAllocator::allocatedHead; pom != nullptr; pom = pom->next)
    {
        //printInteger(pom->size);
    }
}
...
void* MemoryAllocator::mem_alloc(size_t size) //size mi je blokovima
{
    ...
    //size_t actualSize = size + sizeof(FreeMem);
    //size_t forAllocation = ((actualSize % MEM_BLOCK_SIZE) > 0) ? ((actualSize / MEM_BLOCK_SIZE) + 1)*MEM_BLOCK_SIZE : (actualSize / MEM_BLOCK_SIZE)*MEM_BLOCK_SIZE;
    ...
    for(FreeMem *cur = freeMemHead; cur != nullptr || ((char*)cur + forAllocation) < HEAP_END_ADDR; prev = cur, cur = cur->next) {
        ...
        if((cur->size /*+ sizeof(FreeMem)*/) - forAllocation > 0)
        {
            ...
            if(allocatedHead == nullptr || allocatedHead == freeMemHead)
            {
                allocatedHead = cur;
                cur->next = nullptr;
                cur->size = forAllocation /*- sizeof(FreeMem)*/;
            }
            else
            {
        ...
        else
        {
            ...
            if(allocatedHead == nullptr)
            {
                allocatedHead = cur;
                cur->next = nullptr;
                cur->size = forAllocation /*- sizeof(freeMemHead)*/;
            }
            else
            {
                FreeMem *preth = allocatedHead;
